...
OpenOnErrorFileStream.prototype.emit1 = function (name, cb) {
    return;
};
...
OpenOnErrorFileStream.prototype.write2 = function (rec) {
    ...
    // won't write an empty message to the file
 ...
 * 'zonepath'.
 */
function loadDatasetInfo(m, log, callback)
{
    var args;
    ...
    zfs(args, log, function (err, fds) {
        ...
        for (line in lines) {
            ...
            if (fields.length === 10) {
                ...
                log.debug('loadDatasetQuota(): ignoring line: ' + line);
            }
        }
...
function preloadZoneData(uuid, log, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            loadDatasetInfo(null, log, function (err, dsinfo) {
                if (!err) {
                    data.dsinfo = dsinfo;
                }
                cb(err);
    ...
    zoneadm(args, log, function (err, fds) {
        ...
        if (err) {
            ...
            if (!errmsg.match(/No such zone configured$/)) {
                ...
                log.error(err, 'getZoneRecords() zoneadm "'
                    + args.join(',') + '" failed');
            }
    ...
    getVmobj(info.zonename, data, log, function (err, vmobj) {
        ...
        loadMetadata(vmobj, log, function (error, metadata, tags) {
            ...
            if (dsinfo.hasOwnProperty(vmobj.zonepath)) {
                ...
                log.debug('no dsinfo for ' + vmobj.uuid);
            }
            ...
            if (vmobj.hasOwnProperty('zfs_filesystem')) {
                ...
                if (dsinfo.hasOwnProperty(delegated)) {
                    ...
                    log.debug('no dsinfo for ' + delegated);
                }
            ...
            for (snap in raw_snapshots) {
                ...
                friendly_name = snap.name.split('@')[1];
                if (!friendly_name) {
                    log.warn('bad snapshot: ' + snap.name);
                    continue;
                }

                matches = friendly_name.match(/^vmsnap-(.*)$/);
                if (matches && matches[1]) {
                    friendly_name = matches[1];
                    snap_time = new Date(snap.created_at * 1000); // *1000 to ms
                    friendly_snapshots.push({'name': friendly_name,
                        'created_at': snap_time.toISOString()});
                } else {
                    log.debug('ignoring unfriendly ' + snap.name);
                    continue;
                }
            }
    ...
    async.series([
        function (cb) {
            ...
            if (volume.hasOwnProperty('image_uuid')) {
                ...
                args.push('-o', 'refreservation=none', '-V', size + 'M',
                    target);
                zfs(args, log, function (err, fds) {
                    if (err) {
                        cb(err);
        ...
        if (payload.add_nics.hasOwnProperty(nic)) {
            ...
            if (n.ip == 'dhcp') {
                fs.writeFileSync(payload.zonepath + '/root/etc/dhcp.'
                    + n.interface, '');
            } else if (n.ip !== undefined) {
                n.ip.replace(/ */,'').split(',').forEach(function(data) {
                    var part = data.split('/');
                    part[1] = part[1] === undefined
                        ? ' netmask ' + n.netmask
                        : '/' + part[1];
                    fs.writeFileSync(payload.zonepath + '/root/etc/hostname.'
                        + n.interface + vs, part[0] + part[1] + '\n');
                    vs = ':' + ++vc;
                });
            }
            ...
            vc = 0;
            if (n.ip6 == 'dhcp') {
                fs.writeFileSync(payload.zonepath + '/root/etc/dhcp6.'
                    + n.interface, '');
            } else if (n.ip6 !== undefined) {
                fs.writeFileSync(payload.zonepath + '/root/etc/hostname6.'
                    + n.interface, '');
                n.ip6.replace(/ */,'').split(',').forEach(function(data) {
                    vs = ':' + ++vc;
                    fs.writeFileSync(payload.zonepath + '/root/etc/hostname6.'
                        + n.interface + vs, data + '\n');
                });
            ...
            VM.waitForZoneState(zoneobj, 'running', {timeout: 30, log: log},
                ...
                if (err) {
                    if (err.code === 'ETIMEOUT') {
                        log.info('timeout waiting for zone to go to '
                            + '"running"');
                    } else {
                        log.error('unknown error waiting for zone to go'
                            + ' "running"');
                    }
                } else {
                    // zone got to running
...
exports.create_snapshot = function (uuid, snapname, options, callback)
{
    ...
    VM.load(uuid, {log: log}, function (err, obj) {
        ...
        zfs(args, log, function (zfs_err, fds) {
            ...
            log.debug('zfs snapshot stdout: ' + fds.stdout);
            log.debug('zfs snapshot stderr: ' + fds.stderr);
...
function validateFirewall(payload, log, callback)
{
    ...
    firewallVMrun({ payload: payload }, fw.validatePayload, log,
        function (err, res) {
        if (err) {
...
function updateFirewallData(payload, vmobj, log, callback)
{
    ...
    log.debug(firewallOpts, 'Updating firewall information');
    firewallVMrun(firewallOpts, fw.add, log, function (err, res) {
        if (err) {
            log.error(err, 'error adding firewall rules');
        }
