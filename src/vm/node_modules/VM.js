...
function OpenOnErrorFileStream(filename) {
    ...
    this.newStream = function () {
        ...
        watcher = fs.watch(self.path, {persistent: false}, function (evt) {
            if (evt != 'rename') {
                return;
            }
...
OpenOnErrorFileStream.prototype.write1 = function (rec) {
    ...
    if (rec.level >= bunyan.ERROR || log_to_file) {
        ...
        this.emit = function () { stream.emit.apply(stream, arguments); };
        this.end = function () { stream.end.apply(stream, arguments); };
        this.once = function () { stream.once.apply(stream, arguments); };
    ...
    if (PAYLOAD_PROPERTIES.hasOwnProperty(prop)) {
        ...
        case 'integer':
            if (value === undefined || trim(value.toString()) === '') {
                ...
                if ((['vnc_port', 'spice_port'].indexOf(prop) !== -1)
                    && (value.toString() === '-1')) {

                    // these keys allow '-1' as a value, so we succeed here even
                    // though we'd otherwise fail.
                    break;
                }

                errors.bad_values.push(prop);
            } else if (prop === 'max_swap' && value < MINIMUM_MAX_SWAP) {
                errors.bad_values.push(prop);
            }
            break;
        case 'integer-8bit':
...
function validateImages(payload, errors, log, callback)
{
    var check_images = [];
    var disk_idx;
    var pool;
    ...
    async.forEachSeries(check_images, function (image, cb) {
        ...
        execFile(cmd, args, function (error, stdout, stderr) {
            ...
            if (error) {
                log.error(error, {'exit': error.code,
                ...
                if (data.hasOwnProperty('manifest')) {
                    if (data.manifest.type !== image.type) {
                        // image is wrong type
                        log.error('image ' + image.uuid + ' is type '
                    ...
                    log.error('cannot find \'manifest\' for image '
                        + image.uuid);
                    errors.bad_values.push(image.property);
                }
            }
...
exports.validate = function (brand, action, payload, options, callback)
{
    ...
    getZpools(log, function (err, zpools) {
                    ...
                    if (typeof (payload[prop][idx]) === 'object') {
                        ...
                        for (subprop in payload[prop][idx]) {
                            ...
                            validateProperty(brand, prefix + subprop, value,
                                subprop_action, zpools, errors, log);
                        }
        ...
        validateImages(payload, errors, log, function () {
    ...
    if (XML_PROPERTIES.hasOwnProperty(where)) {
        ...
        for (prop in attrs) {
            if (XML_PROPERTIES.nic.hasOwnProperty(prop)) {
                ...
                log.debug('unknown net prop: ' + prop);
            }
        }
        ...
        if (XML_PROPERTIES.nic.hasOwnProperty(attrs.name)) {
            ...
            log.debug('unknown net prop: ' + attrs.name);
        }
        ...
        for (prop in attrs) {
            if (XML_PROPERTIES.disk.hasOwnProperty(prop)) {
                ...
                log.debug('unknown disk prop: ' + prop);
            }
        }
        ...
        if (XML_PROPERTIES.disk.hasOwnProperty(attrs.name)) {
            ...
            log.debug('unknown disk prop: ' + attrs.name);
        }
        ...
        log.debug('unknown property: ' + where + ': '
...
function getVmobj(zonename, preload_data, log, callback)
{
    var filename = '/etc/zones/' + zonename + '.xml';
    var parser = new expat.Parser('UTF-8');

    assert(log, 'no logger passed to getVmobj()');
    ...
    fs.readFile(filename, function (error, data) {
        ...
        parser.on('startElement', function (name, attrs) {
            return startElement(name, attrs, state, log);
        });
...
function zfsList(fields, types, log, callback) {
    ...
    zfs_child = spawn(cmd, args, {'customFds': [-1, -1, -1]});
    log.debug('zfs running with pid ' + zfs_child.pid);
    ...
    zfs_child.on('exit', function (code) {
        log.debug('zfs process ' + zfs_child.pid + ' exited with code: '
            + code);
        if (code === 0) {
...
zfs_list_queue.drain = function () {
    // We use the global log here because this queue is not tied to one action.
    VM.log.trace('zfs_list_queue is empty');
};

function getZfsList(fields, types, log, cb) {
    var sorted_fields;
    var sorted_types;
    var task;
    ...
    try {
        zfs_list_in_progress[task].on('result', cb);
    } catch (e) {
        if ((e instanceof TypeError)
            ...
            // cb() will get called when 'result' is emitted.
        } else {
            cb(e);
        }
    }
...
function loadDatasetInfo(log, callback)
{
    assert(log, 'no logger passed to loadDataset()');
    ...
    getZfsList(
        ['name', 'quota', 'volsize', 'mountpoint', 'type', 'compression',
            'recsize', 'volblocksize', 'zoned', 'creation'],
        ['filesystem', 'snapshot', 'volume'],
        log, callback);
}

function loadTags(vmcfg, log, callback)
{
    var filename;

    assert(log, 'no logger passed to loadTags()');

    if (vmcfg.zonepath) {
        filename = vmcfg.zonepath + '/config/tags.json';
        log.trace('loadTags() loading tags from ' + filename);
        ...
        fs.readFile(filename, function (error, data) {
            ...
            if (error) {
                if (error.code === 'ENOENT') {
                    ...
                    log.error('loadTags() failed to load tags'
                        + ' from ' + filename, error);
                    callback(error);
                    return;
                }
...
function loadMetadata(vmcfg, log, callback)
{
    var filename;

    assert(log, 'no logger passed to loadMetadata()');

    if (vmcfg.zonepath) {
        filename = vmcfg.zonepath + '/config/metadata.json';
        log.trace('loadMetadata() loading metadata from ' + filename);
        ...
        fs.readFile(filename, function (error, data) {
            ...
            loadTags(vmcfg, log, function (err, tags) {
                if (err) {
 ...
 *
 */
function preloadZoneData(uuid, log, callback)
{
    var data = {};

    assert(log, 'no logger passed to preloadZoneData()');

    // NOTE: uuid can be null, in which case we get data for all VMs.

    async.series([
        function (cb) {
            getZoneRecords(uuid, log, function (err, records) {
                if (!err) {
            ...
            loadDatasetInfo(log, function (err, dsinfo) {
                if (!err) {
                    data.dsinfo = dsinfo;
                }
                cb(err);
                ...
                if (z && BRAND_OPTIONS[z.brand].hasOwnProperty('features')
                    ...
                    fs.readFile(filename,
                        ...
                        if (!error) {
                            ...
                            if (pid > 0) {
                                z.pid = pid;
                                log.debug('found PID ' + pid + ' for '
                                    + z.uuid);
                            }
                        }
                        if (error && error.code === 'ENOENT') {
                            // don't return error in this case because it just
                            // didn't exist
                            log.debug('no PID file for ' + z.uuid);
                            zcb();
                        } else {
                            zcb(error);
                        }
        ...
        log.trace('leaving preloadZoneData()');
        callback(err, data);
    });
}

function getZoneRecords(uuid, log, callback)
{
    var args = [];
    var results = {};

    assert(log, 'no logger passed to getZoneRecords()');
    ...
    zoneadm(args, log, function (err, fds) {
        ...
        if (err) {
            ...
            if (errmsg.match(/No such zone configured$/)) {
                ...
                log.error({err: err, stdout: fds.stdout, stderr: fds.stderr},
                    'getZoneRecords() zoneadm "' + args.join(',') + '" failed');
            }
        ...
        for (line in lines) {
            ...
            if (fields.length === 8 && fields[1] !== 'global') {
                ...
                log.trace('loaded: ' + JSON.stringify(obj));
...
function getLastModified(vmobj, log)
{
    var files = [];
    var file;
    var stat;
...
function loadVM(uuid, data, log, callback)
{
    var e;
    var info;

    assert(log, 'no logger passed to loadVM()');
    ...
    getVmobj(info.zonename, data, log, function (err, vmobj) {
        if (err) {
            callback(err);
            return;
        }
        ...
        vmobj.last_modified = getLastModified(vmobj, log);
        ...
        loadMetadata(vmobj, log, function (error, metadata, tags) {
            var d;
            var delegated;
            var disk;
            var ds;
            ...
            if (error) {
                if (vmobj.zone_state !== 'incomplete') {
                    log.debug(error, 'zone is in state incomplete ignoring '
                        + 'error: ' + error.message);
                } else {
            ...
            if (dsinfo.hasOwnProperty('mountpoints')
                ...
                log.trace('found dataset "' + vmobj.zfs_filesystem + '" for '
                    + vmobj.uuid);
            } else {
            ...
            if (vmobj.hasOwnProperty('zfs_filesystem')) {
                ...
                if (dsinfo.datasets.hasOwnProperty(delegated)) {
                    ...
                    if (DISABLED) {
                        // XXX currently only support snapshot on zfs_filesystem
                        if (dsinfo.hasOwnProperty('snapshots')
                    ...
                    log.trace('no dsinfo for delegated dataset: ' + delegated);
                }
            ...
            for (snap in raw_snapshots) {
                ...
                if (matches && matches[1]) {
                    ...
                    if (snap.hasOwnProperty('created_at')) {
                        snap_time = new Date(snap.created_at * 1000); // in ms
                        friendly_snap.created_at = snap_time.toISOString();
                    }
...
exports.load = function (uuid, options, callback)
{
    ...
    preloadZoneData(uuid, log, function (error, data) {
        if (error) {
            if (options.missing_ok && error.code === 'ENOENT') {
                // we're expecting the zone to be gone in this case (eg. delete)
                log.debug('VM ' + uuid + ' does not exist (as expected)');
            } else {
                log.error(error, 'VM.load() failed to get zone record'
                    + ' for ' + uuid);
            }
            callback(error);
        } else {
            loadVM(uuid, data, log, callback);
        }
    });
};
...
exports.lookup = function (search, options, callback)
{
    ...
    preloadZoneData(null, log, function (err, data) {
        ...
        if (quick_ok) {
            ...
            if (!options.full) {
                ...
                function expander(uuid, cb) {
                    loadVM(uuid, data, log, function (e, obj) {
                        if (e) {
                            if (e.code === 'ENOENT') {
                ...
                async.forEachSeries(results, expander, function (e) {
                    if (e) {
                        log.error(e, 'VM.lookup failed to expand results: '
                            + e.message);
                        callback(e);
                    } else {
            ...
            async.forEach(uuids, function (uuid, cb) {
                ...
                loadVM(vmobj.uuid, data, log, function (error, obj) {
                    if (error) {
                        if (error.code === 'ENOENT') {
                            // zone likely was deleted since lookup, ignore
                            cb();
...
function lookupConflicts(macs, ips, vrids, log, callback) {
    ...
    preloadZoneData(null, log, function (err, data) {
        ...
        async.forEach(uuids, function (z_uuid, cb) {
            loadVM(z_uuid, data, log, function (error, obj) {
                var ip;
                var mac;
                var vrid;
                ...
                for (ip in ips) {
                    if (ips[ip] !== 'dhcp'
                        ...
                        log.error('Found conflict: ' + obj.uuid
    ...
    async.series([
        function (cb) {
            ...
            if (volume.hasOwnProperty('image_uuid')) {
                ...
                zfs(['snapshot', snapshot], log, function (err, fds) {
                    if (err) {
                        cb(err);
                    } else {
                        args = ['clone', '-F'];
                        ...
                        args.push('-o', 'refreservation=' + size + 'M');
                ...
                args.push('-o', 'refreservation=' + size + 'M', '-V',
                    size + 'M', target);
...
function saveMetadata(payload, log, callback)
{
    ...
    assert(log, 'no logger passed to saveMetdata()');
...
function writeZoneNetfiles(payload, log, callback)
{
    ...
    for (nic in payload.add_nics) {
        if (payload.add_nics.hasOwnProperty(nic)) {
            ...
            fs.readdirSync(payload.zonepath + '/root/etc/').forEach(function(file) {
                if(file.match(RegExp('^(hostname|dhcp|auto)6?\\.' + n.interface))) {
                    fs.unlinkSync(payload.zonepath + '/root/etc/' + file);
                }
            });

            if (n.hasOwnProperty('ip')) {
              if (['dhcp','dhcp6','auto6'].indexOf(n.ip) >= 0) {
                fs.writeFileSync(payload.zonepath + '/root/etc/' +
                    n.ip + '.' + n.interface, '');
              } else {
                fs.writeFileSync(payload.zonepath + '/root/etc/hostname' +
                   ['.','6.'][n.ip.indexOf(':') + 1 & 1] + n.interface,
                   n.hasOwnProperty('mask_bits') ? n.ip + '/' + n.mask_bits 
                     : n.ip + ' netmask ' + n.netmask )
              }
            }
...
function writeZoneconfig(payload, log, callback)
{
    ...
    log.debug('writing ' + JSON.stringify(data) + ' to /'
...
exports.waitForZoneState = function (payload, state, options, callback)
{
    ...
    function handler(err, obj) {
        ...
        if (obj.newstate === sysevent_state) {
            ...
            VM.lookup({'zonename': obj.zonename}, {full: true, log: log},
                function (error, res) {
                    var handler_retry;
    ...
    // down and return.
    VM.load(payload.uuid, {log: log}, function (err, obj) {
        if (err) {
            watcher.cleanup();
            done();
            callback(err);
...
function createZoneUUID(payload, log, callback)
{
    ...
    if (payload.hasOwnProperty('uuid')) {
        ...
        getZoneRecords(null, log, function (err, records) {
            if (err) {
                callback(err);
            } else {
        ...
        execFile('/usr/bin/uuid', ['-v', '4'], function (err, stdout, stderr) {
            ...
            getZoneRecords(null, log, function (e, records) {
                if (e) {
                    callback(e);
                } else {
                    if (records.hasOwnProperty(payload.uuid)) {
...
function checkPayloadProperties(payload, vmobj, log, callback)
{
    ...
    if (!vmobj) {
        ...
        if (BRAND_OPTIONS[brand].features.check_avail_ram
            ...
            if (payload.available_MiB < payload.ram) {
                ...
                log.debug('checkPayloadProperties() memory ok: '
            ...
            for (nic in payload[field]) {
                if (payload[field].hasOwnProperty(nic)) {
                    ...
                    if ((field === 'add_nics' || field === 'update_nics')
                        ...
                        && !net.isIPv4(n.ip) && !net.isIPv6(n.ip)) {
    ...
    async.series([
        function (cb) {
            ...
            validateFirewall(payload.firewall, log, cb);
        }
    ], function (err) {
...
function markProvisionedWhenHWSetup(vmobj, options, cb)
{
    ...
    ival_handle = setInterval(function () {
        VM.load(vmobj.uuid, {log: log}, function (err, obj) {
...
exports.markVMFailure = function (m, options, cb)
{
    var log;
    ...
    if (options.hasOwnProperty('log')) {
        ...
        log = VM.log.child({action: 'markVMFailure', vm: m.uuid});
    }
    ...
    execFile('/usr/bin/ptree', ['-z', m.zonename],
        function (ptree_err, stdout, stderr) {
            ...
            if (ptree_err) {
                log.error(ptree_err, 'unable to get ptree from ' + m.uuid
                    + ': ' + stderr);
            } else {
                log.warn('processes running in ' + m.uuid
                    + ' at fail time:\n' + stdout);
            }
            log.warn(ptree_err, 'zone setup failed, zone is being stopped '
            ...
            zonecfg(['-u', m.uuid, zcfg], log, function (zonecfg_err, fds) {
                ...
                VM.unsetTransition(m, {log: log}, function (unset_err) {
                    if (unset_err) {
                        log.error(unset_err);
                    }

                    VM.stop(m.uuid, {force: true, log: log},
                        function (stop_err) {

                        // only log errors because there's nothing to do

                        if (stop_err) {
                            log.error(stop_err, 'failed to stop VM ' + m.uuid
                                + ': ' + stop_err.message);
                        }
    ...
    if (payload.hasOwnProperty('add_nics')) {
        for (n in payload.add_nics) {
            if (payload.add_nics.hasOwnProperty(n)) {
                ...
                // if nics.*.ip has cidr notation, split mask bits from ip
                if (nic.hasOwnProperty('ip')) {
                  if (nic.ip.indexOf('/') > -1) {
                    tmp = nic.ip.split('/');
                    payload.add_nics[n].ip   = tmp[0];
                    payload.add_nics[n].mask_bits = tmp[1];
                  }
                }
...
function normalizePayload(payload, vmobj, log, callback)
{
    ...
    VM.validate(brand, action, payload, {log: log}, function (errors) {
        ...
        system.getProvisionableMemory(function (err, available_MiB) {
            if (err) {
                log.warn('unable to determine system memory '
...
exports.unsetTransition = function (m, options, callback)
{
    var log;
    ...
    if (options.hasOwnProperty('log')) {
        ...
        log = VM.log.child({action: 'unsetTransition', vm: m.uuid});
    }

    zonecfg(['-u', m.uuid, 'remove attr name=transition'], log,
        function (err, fds) {
        ...
        zonecfg(['-u', m.uuid, 'info attr name=transition'], log,
            ...
            if (m.transition_to && m.transition_to === 'start') {
                log.debug('VM ' + m.uuid + ' was stopping for reboot, '
                    + 'transitioning to start.');
                VM.start(m.uuid, {}, {log: log}, function (e) {
                    if (e) {
...
function setTransition(m, transition, target, timeout, log, callback)
{
    ...
    async.series([
        function (cb) {
            if (m.hasOwnProperty('transition')) {
                VM.unsetTransition(m, {log: log}, cb);
            } else {
                cb();
            }
        }, function (cb) {
        ...
        function (cb) {
            ...
            if (vmobj.state !== 'stopped') {
                ...
                VM.stop(uuid, {log: log}, function (e) {
                    ...
                    VM.load(uuid, {log: log}, function (error, obj) {
                        if (error) {
    ...
    async.series([
        ...
        // TODO: replace these next two with VM.stop(..{force: true} ?
            ...
            if (zoneobj && zoneobj.hasOwnProperty('disks')) {
                async.forEachSeries(zoneobj.disks, loggedDeleteVolume,
                    function (err) {
                        if (err) {
            ...
            VM.load(uuid, {log: log, missing_ok: true}, function (err, obj) {
                ...
                if (err && err.message.match(gone)) {
                    ...
                    // there was a non-expected error.
                    cb(err);
                } else {
                    // the VM still exists!
            ...
            fs.unlink('/etc/zones/' + zoneobj.uuid + '-receiving.json',
                function (e) {
                    ...
                    if (e && e.code !== 'ENOENT') {
                        log.warn(e, 'Failed to delete ' + zoneobj.uuid
...
function startZone(zoneobj, log, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            // ensure it booted
            VM.waitForZoneState(zoneobj, 'running', {timeout: 30, log: log},
                function (err, result) {
...
function updateZonecfgTimestamp(obj, callback)
{
    var file;
    var now;

    assert(obj.zonename, 'updateZonecfgTimestamp() obj must have .zonename');

    file = path.join('/etc/zones/', obj.zonename + '.xml');
    now = new Date();
...
exports.rollback_snapshot = function (uuid, snapname, options, callback)
{
    ...
    VM.load(uuid, {log: log}, function (err, obj) {
        ...
        found = false;
        if (obj.hasOwnProperty('snapshots')) {
            for (snap in obj.snapshots) {
                if (obj.snapshots[snap].name === snapname) {
                    found = true;
                    break;
                }
            }
        ...
        snapshot_list = [obj.zfs_filesystem + '@vmsnap-' + snapname];

        if (obj.state !== 'stopped') {
            VM.stop(obj.uuid, {'force': true, log: log}, function (stop_err) {
                if (stop_err) {
                    log.error(stop_err, 'failed to stop VM ' + obj.uuid
                        + ': ' + stop_err.message);
                    callback(stop_err);
                    return;
                }
                performSnapshotRollback(snapshot_list, log,
                    ...
                    if (options.do_not_start) {
                        ...
                        VM.start(obj.uuid, {}, {log: log}, callback);
                    }
                    return;
                });
            });
...
exports.delete_snapshot = function (uuid, snapname, options, callback)
{
    ...
    VM.load(uuid, {log: log}, function (err, obj) {
        ...
        args = ['destroy', obj.zfs_filesystem + '@vmsnap-' + snapname];
        zfs(args, log, function (zfs_err, fds) {
            if (zfs_err) {
                log.error({'err': zfs_err, 'stdout': fds.stdout,
            ...
            log.debug({err: zfs_err, stdout: fds.stdout, stderr: fds.stderr},
                'zfs destroy ' + obj.zfs_filesystem + '@vmsnap-' + snapname);
            updateZonecfgTimestamp(obj, function (e) {
                if (e) {
...
exports.create_snapshot = function (uuid, snapname, options, callback)
{
    ...
    VM.load(uuid, {log: log}, function (err, obj) {
        ...
        if (obj.hasOwnProperty('datasets') && obj.datasets.length > 0) {
            callback(new Error('Cannot currently snapshot zones that have '
                + 'datasets'));
            return;
        }

        full_snapname = obj.zfs_filesystem + '@vmsnap-' + snapname;

        // Check that name not already used
        if (obj.hasOwnProperty('snapshots')) {
            for (snap in obj.snapshots) {
                snap = obj.snapshots[snap];
        ...
        zfs(args, log, function (zfs_err, fds) {
            ...
            updateZonecfgTimestamp(obj, function (e) {
                if (e) {
                    log.warn(e, 'failed to update timestamp after snapshot');
                }
                callback();
            });
        });
...
exports.start = function (uuid, extra, options, callback)
{
    ...
    VM.load(uuid, {log: log}, function (err, obj) {
        if (err) {
            ...
            lookupInvalidNicTags(obj.nics, log, function (e) {
                if (e) {
                    callback(e);
                    return;
                }
                ...
                if (BRAND_OPTIONS[obj.brand].features.type === 'KVM') {
                    startVM(zoneobj, extra, log, callback);
                } else if (BRAND_OPTIONS[obj.brand].features.type === 'OS') {
                    startZone(zoneobj, log, callback);
                } else {
...
function resizeTmp(zonename, newsize, log, callback)
{
    ...
    var newvfsline = 'swap        -   /tmp                tmpfs    -   yes    '
        + 'size=' + newsize + 'm';
    var vfsreplace = '"s|^swap.*/tmp.*tmpfs.*$|' + newvfsline + '|"';
    ...
    execFile('/usr/sbin/zlogin', args, function (err, mnt_stdout, mnt_stderr) {
        ...
        execFile('/usr/sbin/zlogin', args,
            function (e, sed_stdout, sed_stderr) {
                ...
                if (e) {
                    log.error({'err': e, 'stdout': sed_stdout,
...
function firewallVMrun(opts, fn, log, callback)
{
    assert(log, 'no logger passed to firewallVMrun()');

    VM.lookup({}, {full: true, log: log}, function (err, records) {
        if (err) {
            callback(err);
            return;
        }
...
function validateFirewall(payload, log, callback)
{
    assert(log, 'no logger passed to validateFirewall()');

    log.debug(payload, 'Validating firewall payload');
    firewallVMrun(payload, fw.validatePayload, log,
        function (err, res) {
        if (err) {
...
function addFirewallData(payload, vmobj, log, callback)
{
    var firewallOpts = payload.firewall;

    assert(log, 'no logger passed to addFirewallData()');
    ...
    log.debug(firewallOpts, 'Adding firewall data');
    firewallVMrun(firewallOpts, fw.add, log, function (err, res) {
        if (err) {
            log.error(err, 'Error adding firewall data');
        }
...
function updateFirewallData(payload, vmobj, log, callback)
{
    var enablePrefix = 'En';
    var enableFn = fw.enable;
    var firewallOpts = payload.firewall;

    assert(log, 'no logger passed to updateFirewallData()');
    ...
    log.debug(firewallOpts, 'Updating firewall data');
    firewallVMrun(firewallOpts, fw.update, log, function (err, res) {
        if (err) {
            log.error(err, 'Error updating firewall data');
        }
        ...
        log.debug('%sabling firewall for VM %s', enablePrefix, vmobj.uuid);
        firewallVMrun({ vm: vmobj }, enableFn, log, function (err2, res2) {
            if (err2) {
                log.error(err, 'Error %sabling firewall',
                    enablePrefix.toLowerCase());
            }
...
exports.update = function (uuid, payload, options, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            updateFirewallData(payload, vmobj, log, cb);
        }, function (cb) {
    ...
    VM.load(uuid, {log: log}, function (err, obj) {
        ...
        zoneadm(['-u', uuid, 'halt', '-X'], log, function (e, fds) {
            ...
            if (msg.match(/zone is already halted$/)) {
                ...
                VM.unsetTransition(obj, {log: log}, function () {
                    var new_err;

                    new_err = new Error('VM ' + obj.uuid + ' is already '
                        + 'not \'running\' (currently: ' + obj.state + ')');
...
function reset(uuid, log, callback)
{
    ...
    VM.load(uuid, {log: log}, function (err, obj) {
        ...
        if (BRAND_OPTIONS[obj.brand].features.use_vmadmd) {
            postVmadmd(obj.uuid, 'reset', {}, log, function (e) {
                if (e) {
...
exports.reboot = function (uuid, options, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            lookupInvalidNicTags(m.nics, log, function (e) {
                if (e) {
...
exports.sysrq = function (uuid, req, options, callback)
{
    ...
    VM.load(uuid, {log: log}, function (err, obj) {
        ...
        postVmadmd(obj.uuid, 'sysrq', {'request': req}, log, function (e) {
            if (e) {
