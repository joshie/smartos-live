...
var sl = require('/usr/node/node_modules/syslog');
var spawn = cp.spawn;
 ...
 * has happened or not.
 */
function waitForZoneState(payload, state, options, callback) {
    var sysevent_state;
    var timeout;
    ...
    timeout = setTimeout(function () {
        ...
        watcher.cleanup();
        callback(new Error('timed out waiting for zone to transition to '
            + state));
    }, timeout_secs * 1000);
}
...
function installZone(payload, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            waitForZoneState(payload, 'running', {}, function (err) {
                if (err) {
...
function createZone(payload, callback)
{
    ...
    if (payload.hasOwnProperty('transition')) {
        ...
        // as an option for create's payload.
    ...
    for (property in payload) {
        if (payload.hasOwnProperty(property)) {
            if (PAYLOAD_PROPERTIES.hasOwnProperty(property)
                ...
                // undefined is a special case since we use that to unset properties
...
function buildTransitionZonecfg(transition, target, timeout)
{
    ...
    cmdline = 'add attr; set name=transition; set value="'
        + transition + ':' + target + ':' + (Date.now() + timeout).toString()
        + '"; set type=string; end';
    ...
    child.on('close', function (code) {
        ...
        if (type === 'DATASET') {
            ...
            zfs(['destroy', '-F', chunk_name + '@sending'],
                function (err, fds) {
                    if (err) {
                        VM.log('WARN', 'Failed to destroy ' + chunk_name
                            + '@sending', err);
                    }
                    callback();
                }
            ...
            async.whilst(
                ...
                function (cb) {
                    ...
                    VM.install(result.uuid, function (e) {
                        if (e) {
                            VM.log('WARN', 'could not Install VM: '
                                + e.message, e);
                        }
                        callback(e, result);
                    });
        ...
        async.series([
            function (cb) {
                ...
                zfs(['destroy', '-F', dataset + '@sending'],
                    function (err, fds) {
                        if (err) {
                            VM.log('WARN', 'Unable to destroy ' + dataset
                                + '@sending: ' + err.message, err);
                        }
                        cb(err);
                    }
                );
            ...
            if (err) {
                VM.log('ERROR', 'Failed to send dataset: ' + err.message, err);
            } else {
        ...
        function (cb) {
            ...
            if (vmobj.state !== 'stopped') {
                ...
                VM.stop(uuid, options, function (e) {
                    if (e) {
                        VM.log('ERROR', 'unable to stop VM ' + uuid + ': '
                            + e.message, e);
                        cb(e);
                        return;
                    }
                    waitForZoneState(vmobj, 'installed', {},
                        function (err, result) {
                        ...
                        VM.log('INFO', 'zone transitioned to installed.');

                        VM.load(uuid, function (error, obj) {
                            if (error) {
                                VM.log('ERROR', 'unable to reload VM ' + uuid
                                    + ': ' + error.message, error);
                                return;
                            }
        ...
        function (cb) {
            ...
            fs.unlink('/etc/zones/' + zoneobj.uuid + '-receiving.json',
                function (e) {
                    ...
                    if (e && e.code !== 'ENOENT') {
                        VM.log('WARN', 'Failed to delete ' + zoneobj.uuid
    ...
    zoneadm(['-u', uuid, 'boot', '-X'], function (err, boot_fds) {
        ...
        zonecfg(['-u', uuid, set_autoboot], function (e, autoboot_fds) {
            if (e) {
                ...
                VM.log('ERROR', 'startZone(): Failed to ' + set_autoboot
                    + ' for ' + uuid, e);
            }
...
function startVM(vmobj, extra, callback)
{
    ...
    try {
        ...
        VM.log('WARN', 'Unable to create /startvm script in ' + vmobj.uuid, e);
    ...
    fs.writeFile(path.join(vmobj.zonepath, '/root/tmp/vm.metadata'),
        ...
        function (err) {
            if (err) {
                VM.log('DEBUG', 'FAILED TO write metadata to /tmp/vm.metadata: '
                    + err.message, err);
                callback(err);
            } else {
...
function updateVnicProperties(uuid, vmobj, payload, callback)
{
    ...
    async.forEach(payload.update_nics, function (nic, cb) {
        ...
        if (!needsUpdate) {
            VM.log('DEBUG', 'No spoofing opts updated for nic "' + nic.mac
                + '": not updating');
            cb(null);
            return;
        }
        ...
        function (cb) {
            ...
            if (payload.hasOwnProperty('zfs_root_recsize')
                ...
                zfs(['set', 'recsize=' + payload.zfs_root_recsize,
                    ...
                    if (err) {
                        VM.log('ERROR', 'failed to apply zfs_root_recsize: '
                            + fds.stderr);
            ...
            if (payload.hasOwnProperty('zfs_data_recsize')
                ...
                zfs(['set', 'recsize=' + payload.zfs_data_recsize,
                    ...
                    if (err) {
                        VM.log('ERROR', 'failed to apply zfs_data_recsize: '
                            + fds.stderr);
                        cb(new Error(rtrim(fds.stderr)));
                    } else {
                        cb();
            ...
            if (payload.hasOwnProperty('zfs_root_compression')
                ...
                zfs(['set', 'compression=' + payload.zfs_root_compression,
                    ...
                    if (err) {
                        VM.log('ERROR', 'failed to apply zfs_root_compression: '
                            + fds.stderr);
                        cb(new Error(rtrim(fds.stderr)));
                    } else {
                        cb();
            ...
            if (payload.hasOwnProperty('zfs_data_compression')
                ...
                zfs(['set', 'compression=' + payload.zfs_data_compression,
                    ...
                    if (err) {
                        VM.log('ERROR', 'failed to apply zfs_data_compression: '
                            + fds.stderr);
                        cb(new Error(rtrim(fds.stderr)));
                    } else {
                        cb();
            ...
            if (changed_datasets && newobj.hasOwnProperty('zonename')) {
                fs.utimes('/etc/zones/' + newobj.zonename + '.xml', now, now,
                    function (err) {
                        if (err) {
                            VM.log('WARN', 'Unable to "touch" xml file for "'
...
exports.update = function (uuid, payload, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            normalizePayload(payload, vmobj, cb);
        }, function (cb) {
            checkPayloadProperties(payload, vmobj, function (e) {
